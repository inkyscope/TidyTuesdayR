...
plot <- df_join %>% 
  ...
  geom_density_ridges(scale = 1.5,
                      lwd = 1.1,
                      alpha = .8) +
  annotate("text", label = "Get Caught", size = 5, 
           fontface = "bold", color = "#C70C47",  x = -60,  y = .5, hjust = -0.3, vjust = -1.5) +
  annotate("text", label = "Captured", size = 5,
           fontface = "bold", color = "#291ff7", x = 17, y = 0.5, hjust = -0.3, vjust = -1.5) +
  scale_y_discrete(limits = rev(c("Scooby", "Fred", "Shaggy", "Velma", "Daphnie"))) +
  scale_x_continuous(limit = c (-60, 30)) +
  scale_fill_manual(values = pals) +
  scale_color_manual(values = pals) +
  coord_cartesian(expand = c(0, .5),
                  clip = "off") +
  labs(color = "",
       fill = "",
       x = "",
...
ggsave(here::here("2021", "week29", "W29_ScoobyDoo.png"), width = 36, height = 28, units = "cm", type = "cairo")
```
...
```{r streamline join}
df_new <- scoobydoo %>%
  select(index, season, year, starts_with("caught"), starts_with("captured")) %>%
  pivot_longer(caught_fred:captured_scooby) %>%
  separate(name, c("type", "character")) %>%
  mutate(value = case_when(
    type == "caught" & value == "TRUE" ~ 1,
    type == "captured" & value == "TRUE" ~ -1,
    TRUE ~ 0
  )
  ) %>%
  group_by(character, year) %>% 
